  ...
  try {
    ...
    for (const profile of profiles) {
      try {
        ...
        if (user) {
          ...
          if (profileError) {
            console.log(`⚠️ Profil für ${profile.email}:`, profileError.message);
          } else {
    ...
    const investigations = [
      {
        ...
        description: 'Max Mustermann wurde am 15. Januar 2025 zuletzt in der Innenstadt gesehen.',
    ...
    for (const investigation of investigations) {
      try {
        ...
        if (error) {
          console.log(`⚠️ Fahndung ${investigation.case_number}:`, error.message);
        } else {
          console.log(`✅ Fahndung erstellt: ${investigation.title}`);
        }
      } catch (error) {
        console.log(`⚠️ Fehler bei Fahndung ${investigation.case_number}:`, error.message);
      }
    }
